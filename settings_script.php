...
$query = "SELECT email, password FROM usersignup WHERE email ='" . $_SESSION['email'] . "'";
...
if ($new_pass != $new_pass1) {
    ...
    if ($old_pass == $orig_pass) {
        $query = "UPDATE  usersignup SET password = '" . $new_pass . "' WHERE email = '" . $_SESSION['email'] . "'";
        mysqli_query($con, $query) or die($mysqli_error($con));
        echo '<script type="text/javascript"> alert("Update Successful...log in again"); window.location.href = "/cabv2/logout_script.php"; </script>';
    } else
